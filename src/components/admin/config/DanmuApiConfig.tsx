...
interface DanmuApiConfigSettings {
  enabled: boolean;
...
function DanmuApiConfigContent() {
  const [settings, setSettings] = useState<DanmuApiConfigSettings>({
    enabled: true,
    useCustomApi: false,
    customApiUrl: '',
    customToken: '',
    ...
    const loadConfig = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (config && config.DanmuApiConfig) {
            setSettings({
        ...
        logger.error('加载弹幕API配置失败:', error);
      } finally {
        setLoaded(true);
      }
    };
  ...
  return (
    <div className='bg-purple-50 dark:bg-gray-800 rounded-lg shadow-md p-4 sm:p-6'>
...
export default function DanmuApiConfig() {
  return <DanmuApiConfigContent />;
}
